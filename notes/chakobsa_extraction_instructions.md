...
Write a perl script that can extract the Chakobsa dialogue from the text scripts found in scripts/raw/.  The data needs to be written out as JSON, including the following data:
...
The script should process all the files in scripts/raw/ and output the JSON to a single file at `scripts/chakobsa.json`.  Call this script `extract_chakobsa.pl`.  It should be written in safe perl 5.30 with `strict` and `warnings` pragmas enabled.  It should use the `JSON` module to output the JSON data.  It should be written in a way that is easy to understand and maintain.  It should be well-documented with comments explaining the code.
